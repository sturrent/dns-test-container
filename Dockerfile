...
RUN apt-get update && apt-get install -y dnsutils curl gnupg gnuplot nginx txt2html wget && \
    wget -O - https://pkg.dns-oarc.net/dns-oarc.distribution.key.gpg | tee /etc/apt/keyrings/pkg.dns-oarc.net.asc && \
    echo "deb [signed-by=/etc/apt/keyrings/pkg.dns-oarc.net.asc] http://pkg.dns-oarc.net/debian bookworm main" | tee /etc/apt/sources.list.d/dns-oarc-pr.list && \
