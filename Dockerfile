...
RUN apt-get update && apt-get install -y dnsutils curl gnupg gnuplot nginx txt2html wget && \
    ...
    echo "deb [signed-by=/etc/apt/keyrings/pkg.dns-oarc.net.asc] http://pkg.dns-oarc.net/stable/bookworm bookworm main" | tee /etc/apt/sources.list.d/dns-oarc-pr.list && \
